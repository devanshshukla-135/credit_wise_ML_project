...
st.markdown("Enter applicant details below to check loan eligibility (Naive Bayes Model).")

# 1. Load Pre-trained Model, Scaler & Feature Columns
@st.cache_resource
def load_artifacts():
...
try:
    ...
    st.error(f"Artifacts Load Error: {e}. Notebook me model, scaler aur features properly dump karein.")
    st.stop()

# Mappings for UI Inputs
...
if st.button("Predict Loan Approval Status", use_container_width=True):
    # Features Dictionary Initialized matching trained model structure
    ...
    # Clean Pandas DataFrame creation
    input_df = pd.DataFrame([row])[feature_cols]
    input_df = input_df.astype(float)

    try:
        # Standardize & Predict via Naive Bayes
        ...
        st.subheader("Result:")
        if prediction[0] == 1:
            st.success(f"🎉 Congratulations! Loan is APPROVED (Confidence: {proba[1]*100:.2f}%)")
        else:
            st.error(f"❌ Loan Application REJECTED (Confidence: {proba[0]*100:.2f}%)")
    except Exception as e:
